# AGV 제어 시스템 - State Machine 기반

## 개요
학습용으로 설계된 AGV 제어 시스템입니다. State Machine 패턴을 사용하여 명확하고 이해하기 쉬운 구조로 구성했습니다.
...
```
- **포트**: 5000번 (웹 대시보드)
- **접속**: 브라우저에서 `http://localhost:5000` 또는 `http://<서버IP>:5000`
- **역할**: 
  - State Machine 관리 (서버 측에서 AGV 상태 제어)
  - 웹 대시보드 제공 (실시간 AGV 위치 시각화)
  - 웹 AGV 시뮬레이션 (경로를 따라 부드럽게 이동)
...
```
- **포트**: 5001번 (서버로부터 명령 수신)
- **역할**:
  - 라인 트레이싱 수행
  - RF-Tag 감지 및 임무 실행
  - 서버로 센서 데이터 전송 (0.1초마다)
  - 서버 명령에 따라 모터 제어

### 3단계: 웹 대시보드에서 제어
1. 브라우저에서 `http://localhost:5000` 접속
2. **GO** 버튼 클릭 → AGV가 Running 상태로 전환 및 주행 시작
3. 실시간으로 다음 정보 확인:
   - AGV 위치 (검은색 차량 아이콘)
   - 웹 AGV 시뮬레이션 (경로를 따라 이동)
   - RF-Tag 감지 상태
   - 속도, 배터리, LiDAR 거리

### 웹 대시보드 버튼 설명
- **GO**: 주행 시작 (Ready → Running) 또는 Home 복귀 (Completed → 시작점)
- **PAUSE**: 일시정지 (Running → Paused)
- **RESUME**: 재개 (Paused → Running)
- **E-STOP**: 비상정지 (모든 상태 → SRV-EStop)
- **STOP**: 정지 및 5초 후 Ready 전환
